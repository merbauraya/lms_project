		...
		if (isset($_POST['AcquisitionSuggestionItem']))
                {
                        ...
                        $isNew=true;
                        $itemID = $_POST['AcquisitionSuggestionItem']['id'];
                        echo $itemID;
                        ...
                        {
                           $acqItem = AcquisitionSuggestionItem::model()->findByPk($itemID);
                           $isNew=false;
                        }
                        $acqItem->attributes = $_POST['AcquisitionSuggestionItem'];
                        if ($isNew)
                           unset($acqItem->id);
                        $acqItem->acq_suggestion_id=$id;
                        $acqItem->save(false);
                         /*
				//load posted id so we can delete existing non posted record
				$itemsPK = array();//array_keys($items,'id');
				foreach($items as $i=>$item)
				{
					$itemsPK[$i] = $item['id'];		
				}
				AcquisitionSuggestionItem::model()->deleteOldItems($id,$itemsPK);
				foreach($items as $i=>$item)
					if (isset($items[$i]))
					{
        ...
        array(
             ...
             'pagination' => array(
                 'pageSize' => '20',
                       ...
                       $items = $this->loadSuggestionItems($id);
