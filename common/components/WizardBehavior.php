...
class WizardBehavior extends CBehavior {
	...
	*/
	public function process($step) {
        //mazlan added to save our params passed
	...
	*/
	protected function redirect($step = null, $terminate=true, $statusCode=302) {
		if (!is_string($step))
			$step = $this->getExpectedStep();
        //mazlan edited to store query params during redirect
        $stepA = array($this->queryParam=>$step);
        $route = $this->owner->id.'/'.$this->getOwner()->getAction()->getId();
        $arrParams = array_merge($stepA,$this->params);
		//$url = array($this->owner->id.'/'.$this->getOwner()->getAction()->getId(), $this->queryParam=>$step);
        $url = $this->owner->createUrl($route,$arrParams);
//

        //$url = array($this->owner->id.'/'.$this->getOwner()->getAction()->getId(), $arrParams);
